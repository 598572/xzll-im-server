...
func (c *WebSocketConnection) handleConnection() {
	...
	// 设置读取参数（对标 Java WebSocket 配置）
	c.conn.SetReadLimit(nettyConfig.MaxMessageSize)
	
	// 设置 Pong 处理器（对标 Java PongWebSocketFrame 处理）
	c.conn.SetPongHandler(func(appData string) error {
		// 记录心跳响应（客户端回复 Pong）- 对标 Java recordHeartbeatResponse(ctx, "pong")
		c.recordHeartbeatResponse("pong")
		return nil
	})
	
	// 启动心跳检测协程（对标 Java 心跳定时器）
	// 使用 HeartbeatTimeout（秒）而不是 PongTimeout（毫秒）
	...
	// 主消息循环（对标 Java channelRead 方法）
	for {
		// 设置读取超时
		c.conn.SetReadDeadline(time.Now().Add(nettyConfig.PongTimeout))
		
		messageType, message, err := c.conn.ReadMessage()
		if err != nil {
			if websocket.IsUnexpectedCloseError(err, websocket.CloseGoingAway, websocket.CloseAbnormalClosure, websocket.CloseNormalClosure) {
				c.server.logger.Error("WebSocket 读取错误",
		...
		// 处理不同类型的消息（对标 Java WebSocketServerHandler 消息分发）
		switch messageType {
		case websocket.TextMessage:
			// 文本消息（当前版本不支持，对标 Java 版本只支持 Protobuf）
		...
		case websocket.BinaryMessage:
			// Protobuf 二进制消息（主要消息类型）
			// 对标 Java: 业务消息也会更新 readerTime
			c.recordHeartbeatResponse("binary")
			c.server.messageHandler.HandleBinaryMessage(c, message)
			
		case websocket.PingMessage:
			// Ping 消息，回复 Pong（对标 Java PingWebSocketFrame 处理）
			// 对标 Java: 收到客户端主动 Ping，更新 readerTime + 记录心跳响应
			c.recordHeartbeatResponse("ping")
			if err := c.SendPong(message); err != nil {
				c.server.logger.Error("发送 Pong 失败",
					zap.String("user_id", c.userID),
					zap.Error(err),
				)
				return
		...
		case websocket.PongMessage:
			// Pong 消息已在 SetPongHandler 中处理（已调用 recordHeartbeatResponse）
			
		case websocket.CloseMessage:
			// 关闭消息
			c.server.logger.Info("收到关闭消息",
				zap.String("user_id", c.userID),
			)
			return
		}
