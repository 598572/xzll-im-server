...
@Slf4j
public class WebsocketClient555 {
    ...
    private static WebsocketClientHandler555 handler;

    public static volatile boolean getMsgFlag = false;

    public WebsocketClient555(String ip, int port) {
        this.ip = ip;
        this.port = port;
    ...
    public void run() throws InterruptedException, URISyntaxException {
        ...
        handler = new WebsocketClientHandler555(webSocketClientHandshaker);
...
//                ch.attr( AttributeKey.valueOf(USER_ID)).set(RandomUtil.randomNumbers(2));
        ...
        new Thread(() -> {
            ...
            while (true) {
                ...
                if (!getMsgFlag && CollectionUtils.isEmpty(WebsocketClientHandler555.msgIds)) {
...
//                String s="你好啊 我发消息压死你，第"+i+"条消息";
                ...
                if (StringUtils.isNotBlank(s)) {
                    String msgId = WebsocketClientHandler555.msgIds.remove(0);
